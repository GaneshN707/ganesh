...
def Show_data(request):
    ...
    if request.method == "POST":
        # Retrieve the medicine details from the POST request
        ...
        medicine_mrp = request.POST['medicine_mrp']
        
        # Save the medicine information to the database
        med = medicine(
            ...
            Maximum_Retail_prise=medicine_mrp,
            user=request.user,  # Assign the user to the medicine entry
            city=Medicines.city,
            state=Medicines.state,
            postal_code=Medicines.postal_code,
        )
        med.save()
        messages.success(request, "Medicine information has been saved.")
        context['med'] = med
...
def scaned_qr(request, qr_value):
    ...
    if qr_value in chain_data:
        ...
        if isinstance(associated_value, list):
            ...
            data = f"{user_firm.user_type}#{user_firm.city}#{user_firm.state}#{user_firm.country}"
...
def trace_qr(request, qr_value):
    ...
    if qr_value in chain_data:
        ...
        messages.success(request, "Authorized Qr Code")
        return render(request, "trace_qr.html",context)
